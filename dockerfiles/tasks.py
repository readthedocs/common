...
def manage(c, command, running=True, backupdb=False):
    ...
    if backupdb:
        c.run(
            f'{DOCKER_COMPOSE_COMMAND} {subcmd} database pg_dumpall -c -U docs_user > dump_`date +%d-%m-%Y"_"%H_%M_%S`__`git rev-parse HEAD`.sql',
            pty=True,
        )
...
def attach(c, container):
    ...
    c.run(
        f'docker attach --sig-proxy=false --detach-keys="ctrl-p,ctrl-p" {prefix}-{container}-1',
        pty=True,
    )
...
def test(c, arguments="", running=True):
    ...
    if running:
        c.run(
            f"{DOCKER_COMPOSE_COMMAND} exec -e GITHUB_USER=$GITHUB_USER -e GITHUB_TOKEN=$GITHUB_TOKEN web uv run tox {arguments}",
            pty=True,
        )
    else:
        c.run(
            f"{DOCKER_COMPOSE_COMMAND} run -e GITHUB_USER=$GITHUB_USER -e GITHUB_TOKEN=$GITHUB_TOKEN --rm --no-deps web uv run tox {arguments}",
            pty=True,
        )
...
def compilebuildtool(c, tool, version, os=None):
    ...
    if version not in valid_versions:
        print(
            f"Invalid version for the specified tool. You must specify one of {', '.join(valid_versions)}"
        )
        sys.exit(1)
