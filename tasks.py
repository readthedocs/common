...
def setup_labels(ctx, repo, dry_run=False):
    ...
    for label in labels:
        ...
        # Look for exact match first, then transposable labels
        ...
        # We have a label to modify, update the label
        if found:
            # Cool, description is never updated. We'll just force the update
            print('Updating label: {0}'.format(found.name))
            if not dry_run:
                found.edit(label.name, label.color, label.desc)
        # We have to create the label instead
        else:
            print('Creating label: {0}'.format(label.name))
            if not dry_run:
                repo.create_label(label.name, label.color, label.desc)

    # Second pass: delete bad labels, look for untransposed labels (manual
    # invention is required here), and note any foreign labels
    untouched = set()
